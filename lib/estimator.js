...
export function estimateCabinetCost(input) {
  ...
  // Support both old 'sheetRates' key and new separate keys from MySQL
  const sheetRates   = input.sheetRates || DEFAULT_SHEET_RATES
  const baseRates    = input.baseRates  || {}
  const feeInclusive = input.feeInclusiveRates || {}
  ...
  const resolveRate = (category) => {
    // New MySQL config path: separate baseRates + feeInclusiveRates
    const feeKey = includeFees ? feeInclusive : baseRates
    if (feeKey && feeKey[category] != null) {
      const r = Number(feeKey[category])
      if (Number.isFinite(r) && r > 0) return r
    }
    // Legacy sheetRates path (backward compat)
